...
#     #executor = submitit.LocalExecutor(folder=args.output_dir)
#     kwargs = {}
#     if args.use_volta32:
...
def submit_jobs(task_class, args, name: str):
    ...
    # Set up the torchrun command for distributed training
    torchrun_command = [
        "torchrun",
        "--nnodes", str(os.environ.get("WORLD_SIZE", 1)),  # Set world size
        "--nproc_per_node", "8",  # Fixed: set nproc_per_node to 8
        "--rdzv_backend", "c10d",
        "--rdzv_endpoint", f"{os.environ.get('MASTER_ADDR', '127.0.0.1')}:{os.environ.get('MASTER_PORT', 29500)}",
        "--rdzv_id", "dinov2_training_script",
        "--node_rank", str(os.environ.get("NODE_RANK", 0)),
        "dinov2/run/train/train.py"
    ]
    # Execute the torchrun command using subprocess
    try:
        task = task_class(args)  # Initialize the task object
        logger.info(f"Starting training with torchrun: {' '.join(torchrun_command)}")
        subprocess.run(torchrun_command, check=True)
        task()  # Call the task function to start training
    except subprocess.CalledProcessError as e:
        logger.error(f"torchrun failed with error: {e}")
        raise
    # Logging output directory for checkpoints and logs
