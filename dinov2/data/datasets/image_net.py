...
class _Split(Enum):
    ...
    def length(self) -> int:
        split_lengths = {
            _Split.TRAIN: 1_281_169,    #let me change this from 1_281_167,
            _Split.VAL: 2_000,          #let me change this from 50_000,   
            _Split.TEST: 100_000,
        }
...
class ImageNet(ExtendedVisionDataset):
    ...
    def __len__(self) -> int:
        entries = self._get_entries()
        #import numpy as np
        #np.save(f"/rsrch1/ip/msalehjahromi/codes/dinov2-main/yamls/zzz_{str(len(entries))}_entries_in_train.npy", np.array([1,2]))
        #np.save(f"/rsrch1/ip/msalehjahromi/codes/dinov2-main/yamls/zzz_{str(self.split.length)}_split_in_train.npy", np.array([1,2]))
